...
def create_database(labels, sample_rate, total_windows_in_file_max, max_value,
    ...
    print(f'* created h5file: {h5file}')
    print(f'* now creating datasets for each of the following categories: folder, class, score, gender, index, features')
    ...
    print(f'* successfully created datasets in the database of the h5file')
    ...
    # Feature extractor
    print(f'* now running feature extractor...')
    print(f'* features_exp = {features_exp}')
    if features_exp == 'logmel' or features_exp == 'mel':
        if features_exp == 'mel':
            log = False
        else:
            log = True
        print(f'* caulling LogMelExtractor...')
        feature_extractor = audio_feature_extractor.LogMelExtractor(
            ...
            log=log, snv=snv)
        print(f'* successfully returned from LogMelFeatureExtractor...')
        print(f'* feature_extractor is of type: {type(feature_extractor)}')

    # Loop through all the files and get the sampling rate, number of
    # samples and the time in minutes
    print(f'* now looping through files to get the sample rate, num samples, and time in mins')
    num_samples_feature = []
    for pointer, folder in enumerate(labels[0]):
        ...
        if features_exp == 'logmel' or features_exp == 'mel':
            ...
            print(f'* mel_spec has type: {type(mel_spec)}')
            print(f'* attempting to plot now...')
    ...
    print(f'* reached the end of h5file, closing now...')
    h5file.close()
    print(f'* closed the h5file... returning num_samples_feature, a list of len: {len(num_samples_feature)}')
...
def process_organise_data(main_logger,
    ...
    print(f'* min/max values logged, fixing test files now...')
    utilities.fix_test_files()
    print(f'* merging CSVs now...')
    ...
    print(f'* CSVs merged, now getting labels from dataframe based on path {config.COMP_DATASET_PATH}')
    labels = utilities.get_labels_from_dataframe(config.COMP_DATASET_PATH)
    print(f'* labels accessed, now splititng into genders if that is enabled in config...')
    ...
    # labels = [l1, l2, l3, l4]
    if config.GENDER:
        print(f'* filtering labels by gender...')
        fin_label = [[[], [], [], []], [[], [], [], []]]
        ...
        gender = ['f', 'm']
        for i in range(2):
            print(f'* creating gender-specific database for both genders')
        ...
        print(f'* creating database irrespective of gender')
    ...
    print(f'* summary labels: {summary_labels}\n')


    print(f'* saving summary values now...')
    summary_values = [max_value, total_windows_in_file_max, min_value,
                      total_windows_in_file_min, sample_rate, len(labels[0]),
                      num_samples_feature]

    print(f'* summarized the labels and values... now saving to .pickle file...')
    ...
    try:
        ...
        print(f'successfully saved summary to path: {save_path}')
    except Exception as e:
        print(f'* ERROR OCCURED WHEN SAVING SUMMARY.PICKLE FILE')
        raise e
    print(f'finished attempting to save summary to {save_path}')

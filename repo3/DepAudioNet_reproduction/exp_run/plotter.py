...
def plot_graph(epoch, results, total_epochs, model_dir, early_stopper=False,
    ...
    """
    print(f'* running plot_graph...')
    x_values = list(range(1, epoch+1))

    print(f'* running plt.subplots')
    fig, ax1 = plt.subplots()
    ax2 = ax1.twinx()
    print(f'* made it past plt.subplots')

    # y = results['train_loss'].tolist() # selena added to debug
    # print(f'* x values: (x): {type(x_values)} results: (y): {type(y)}')
    # print(f'* axis lengths: x: {len(x_values)} y: {len(y)}')
    # print(f'* x: {x_values}')
    # print(f'* y: {y}')

    y_mean_fscore = results['train_mean_fscore'].tolist()
    y_mean_acc = results['train_mean_acc'].tolist()
    print(f'* additional stats: len mean fscore: {len(y_mean_fscore)} len mean acc: {len(y_mean_acc)}')
    ...
    fig.set_figwidth(15)
    print(f'* vis is T/F? vis = {vis}')
    if vis and not early_stopper and epoch % 20 == 0:

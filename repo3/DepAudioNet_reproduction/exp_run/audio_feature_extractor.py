...
def feature_segmenter(feature, meta, feature_exp, dim, convert_to_image=False):
    ...
    """
    # selena added this initial if statement:
    if feature.shape[1] == 0:
        print(f'* UH OH: feature shape is {feature.shape}, wich means it has {feature.shape[0]} rows that are all empty')
        print(f'* the feature array contains: \n{feature}')
        print(f'* this will probably cause a division by 0 error')
        print()
    if feature.shape[1] % dim == 0:
        num_extra_dimensions = (feature.shape[1] // dim)
        # print(f'* num_extra_dimensions = feature.shape[1] {feature.shape} // {dim} = num_extra_dimensions')
    else:
        num_extra_dimensions = (feature.shape[1] // dim) + 1
        # print(f'* num_extra_dimensions = feature.shape[1] {feature.shape} // {dim} +1')
    ...
    if convert_to_image:
        ...
        if leftover != dim:
            feature = np.hstack((feature, np.zeros((feature.shape[0],
                                                    leftover))))
        # print(f'* meta: {meta}')
        # print(f'* interstingly, meta of participant 303 does NOT exist in full_test_split.csv')
        # print(f'* feature has shape: {feature.shape} num_extra_dimensions = {num_extra_dimensions}')
        # print(f'* num_extra_dimensions here is {num_extra_dimensions} leading to a division by 0 error...')
        new_features[:, :, :] = np.split(feature, num_extra_dimensions,
                                         axis=1)

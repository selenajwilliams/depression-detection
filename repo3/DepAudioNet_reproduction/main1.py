...
def setup(current_dir, model_dir, data_type='', path_to_logger_for_test=None):
    ...
    checkpoint = False
    next_exp = False
    print(f'Checking path {features_dir}')
    ...
    if os.path.exists(current_dir) and os.path.exists(model_dir):
        print(f'* model dir: {model_dir} already exists, deleting it and storing contents in tmp dir before continuing')
        ...
        print(f'* curr dir already exists: {current_dir}')
        print(f'* attempting to create model dir at dir: {model_dir}...')
        os.mkdir(model_dir)
    ...
    print(f'* any previous directories successfully handled. creating model now...')
    model = create_model()
...
def train(model, workspace_files_dir):
    ...
    Input
        ...
        workspace_files_dir: str - Location of the programme code
    """
    print(f'* running train in main1.py')
    num_of_classes = len(config_dataset.LABELS)
    learning_rate = config.EXPERIMENT_DETAILS['LEARNING_RATE']
    print(f'model has type: {type(model)} we suspect it is nonetype with the new config, causing errors')
    ...
    if checkpoint:
        print(f'* checkpoint occured, loading prev model outputs & values')
        ...
        print(f'* the dataframe was loaded to be: \n{df.describe()}')
        print(f'* dataframe shape: {df.shape}')
        df.to_csv('tmp.csv', index=True)
    else:
        print(f'* not a checkpoint, initializing model values to empty values')
        ...
        data_saver = {}
        print(f'* the dataframe was initialized to be: \n{df.describe()}')
        print(f'* dataframe shape: {df.shape}')
    ...
    gen, train_info = organiser.run_train(config,
                                          ...
                                          data_saver)
    if checkpoint:
        print(f'* in train function, optimizer is: {optimizer}')
        print(f'''* passing the following args into util.load_model: \n
checkpoint run: \n{checkpoint_run} 
\n model: \n{model} 
\n optimizer: \n{optimizer}
\n cuda: {cuda}
        ...
        if start_epoch != df.shape[0]:
            raise AttributeError(f'''* You are loading a checkpoint of a saved model which has completed {start_epoch} epochs so far, but \n
                                 your saved model variables have saved data for {df.shape[0]} total epochs. This is likely because you  \n
                                 ran an experiment which is overwriting previous results without deleting them. Please delete the  \n
                                 model dir {model_dir} and start again -- or modify the plotter to avoid shapes errors
                                ''')
        else: 
            print(f'* start epoch: {start_epoch}, model variables shape: {df.shape} (expect (n_epochs, 30)')
    else:
        start_epoch = 0
        # train_acc, train_fscore, train_loss, val_acc, val_fscore, val_loss
...
def test():
    ...
    for exp_num in range(exp_runthrough):
        ...
        print(f'* in test function, optimizer is: {optimizer}')
...
if __name__ == '__main__':
    ...
    if args.mode == 'train':
        for i in range(exp_runthrough):
            ...
            print(f'* the model dir is: {model_dir} and validPath = {os.path.exists(model_dir)}')
            ...
            print(f'* before calling train func, model has type: {type(model)}')
            print(f'* before calling train, the workspace files dir is: \n{config.WORKSPACE_FILES_DIR}')
            train(model,
                  config.WORKSPACE_FILES_DIR)

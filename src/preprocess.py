...
def process_data(in_dir, out_dir, set_type, IDs_list):
    ...
    for idx, ID in enumerate(IDs_list):
        ...
        print(f'successfully pre processed all modalities for participant with ID {ID}, containing {round(total_size, 4)} MB, item {idx+1} out of {len(IDs_list)}')
        # write the participant ID to a file to verify that the data has been written successfully! 
        # this way we can avoid repeat computation and be sure that we don't have corrupted, half-written to files
...
def main():
    ...
    """
    init_path = "/oscar/data/rbalestr/selena/data/" # TODO: fix this
    init_directories(home_path=init_path)
    train_set_IDs, dev_set_IDs, test_set_IDs = get_train_dev_test_IDs()

    # print(f'processing training data...')
    in_dir = "/oscar/data/rbalestr/selena/data/raw_data"
    out_dir = "/oscar/data/rbalestr/selena/data/processed_data"
    process_data(in_dir, out_dir, 'train', train_set_IDs)
